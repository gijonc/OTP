...
//##################### Check Inputs #####################		
  ...
  //##################### Reading files and info ####################   
    read_file(argv[1],message,1);
    read_file(argv[2],key,0);
    msglen = strlen(message);
    keylen = strlen(key);  
    if(keylen < msglen){    //check length of key
  ...
  //##################### writing & reading to socket ####################   
    ...
    //receiving ciphertext from server
    n = read(sockfd,ciphertext,MAX_BUF_SIZE-1);
    if (n < 0) 
         error("ERROR reading from socket");
    ...
    while(fgets(buffer, MAX_BUF_SIZE, file) != NULL) {     //fgets each buffer until end
      if (buffer[0] != '\n'){      //detect any emtpy buffer
          ...
          if (leng > 0 && buffer[leng-1] == '\n'){   //check new buffer char \r\n
            if(id==1)
              buffer[leng-1] = 'e';   //buffer as message, set e as id
            else
              buffer[leng-1] = 0;
          }
      }
